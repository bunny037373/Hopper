...
// ================== ⚙️ WORD LISTS & STATIC FILTER CONFIGURATION ⚙️ ==================
...
// Combined list used for Nickname Moderation
const BAD_WORDS = [...MILD_BAD_WORDS, ...SEVERE_WORDS]; 

/**
 * Checks a message against the static word lists (Severe, Mild, and Whitelist).
 * It replaces whitelisted words before checking for bad words to prevent false positives.
 * @param {string} text The message content.
 * @returns {string} Severity ('None', 'Mild', 'Severe').
 */
function checkStaticToxicity(text) {
    let checkText = text.toLowerCase();

    // 1. Sanitize the text by removing whitelisted words first.
    for (const allowed of ALLOWED_WORDS) {
        // Use a regex to replace all occurrences of the allowed word (globally) with spaces
        // This is a simple but effective way to handle false positives.
        checkText = checkText.replace(new RegExp(allowed, 'g'), ' '); 
    }
    
    // Normalize spacing after replacement
    checkText = checkText.replace(/\s+/g, ' '); 

    // 2. Check for SEVERE words
    for (const severe of SEVERE_WORDS) {
        if (checkText.includes(severe)) {
            return 'Severe';
        }
    }

    // 3. Check for MILD words
    for (const mild of MILD_BAD_WORDS) {
        if (checkText.includes(mild)) {
            return 'Mild';
        }
    }
...
// ================= END WORD LISTS & STATIC FILTER CONFIGURATION =================
...
// Helper: Moderate Nickname (Now uses the combined BAD_WORDS list)
async function moderateNickname(member) {
  const NICKNAME_FILTER_WORDS = BAD_WORDS; // Use the comprehensive list
...
async function runAutomatedNicknameScan(guild) {
    ...
    try {
        // Only fetch members if we need to perform the check
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'say') {
      ...
      // Check static filter first for commands
      const staticSeverity = checkStaticToxicity(text);
      if (staticSeverity !== 'None') {
           return interaction.reply({ content: `❌ That message violates the static content filter (${staticSeverity} severity).`, ephemeral: true });
      }

      // Check AI filter second for commands
    ...
    if (interaction.commandName === 'ai') {
        ...
        const staticSeverity = checkStaticToxicity(prompt);
        if (staticSeverity !== 'None') {
            return interaction.editReply(`❌ Your request was blocked by the static content filter (${staticSeverity} severity).`);
        }
        
        // AI check runs as the final safety layer
        const { isToxic: promptIsToxic } = await checkMessageToxicity(prompt);
        if (promptIsToxic) {
             return interaction.editReply('❌ Your request was blocked by the AI safety filter. Please rephrase your question.');
        }
    ...
    if (interaction.commandName === 'sayrp') {
      ...
      // Check static filter first
      const staticSeverity = checkStaticToxicity(message);
      if (staticSeverity !== 'None') {
           return interaction.reply({ content: `❌ That message violates the static content filter (${staticSeverity} severity).`, ephemeral: true });
      }

      // Check AI filter second
...
client.on('messageCreate', async (message) => {
  ...
  if (lowerContent.startsWith('?afk')) {
      ...
      // Static check for the reason
      const staticReasonSeverity = checkStaticToxicity(reason);
      if (staticReasonSeverity !== 'None') {
          await message.delete().catch(() => {});
          return message.channel.send(`❌ <@${userId}>: Your AFK reason was blocked by the static filter.`);
      }

      // AI check for the reason
      const { isToxic } = await checkMessageToxicity(reason);
      if (isToxic) {
          await message.delete().catch(() => {});
          return message.channel.send(`❌ <@${userId}>: Your AFK reason was blocked by the AI safety filter.`);
      }
  ...
  // --- START MODERATION BLOCK (Excluding Image Channel) ---
  if (message.channel.id !== TARGET_CHANNEL_ID && !isPureGIFLink) {
    
    // =========================================================
    // 🟢 LAYER 1: STATIC WORD FILTER (Immediate, Dictionary-based)
    // =========================================================
    const staticSeverity = checkStaticToxicity(content);
    
    if (staticSeverity === 'Severe') {
        await message.delete().catch(() => {});
        try {
            if (member && member.manageable) {
                // Apply 60-minute timeout for Severe Static violations
                await member.timeout(60 * 60 * 1000, `Static Filter Detected Severe Violation`).catch(() => {}); 
            }
            const log = client.channels.cache.get(LOG_CHANNEL_ID);
            if (log) log.send(`🚨 **Static Filter Violation (Timeout 60m)**\nUser: <@${message.author.id}>\nReason: Severe word/slur detected.\nContent: ||${message.content}||`);
        } catch (e) {
            console.error("Failed to apply Severe static moderation action:", e);
        }
        return; // STOP EXECUTION
    }

    if (staticSeverity === 'Mild') {
        await message.delete().catch(() => {});
        const log = client.channels.cache.get(LOG_CHANNEL_ID);
        if (log) log.send(`⚠️ **Static Filter Violation (Deletion)**\nUser: <@${message.author.id}>\nReason: Mild bad word detected.\nContent: ||${message.content}||`);
        return; // STOP EXECUTION
    }

    // =========================================================
    // 🟡 LAYER 2: AI TOXICITY CHECK (Contextual, Bypass-focused)
    // ONLY runs if the static filter was 'None'
    // =========================================================
    const { isToxic, blockCategory } = await checkMessageToxicity(content);

    // RULE: INAPPROPRIATE RP LOCKDOWN (Uses AI for contextual check)
    if (message.channel.id === RP_CHANNEL_ID && isToxic) {
        ...
        if (category && category.type === 4) { 
            try {
                ...
                if (everyoneRole) {
                    // Lock down the entire RP category
    ...
    // --- AI MODERATION ACTION (Timeout for general channels) ---
    if (isToxic) {
      await message.delete().catch(() => {});
      
      try {
        if (member && member.manageable) {
            // Apply 30-minute timeout for AI-detected toxicity
    ...
    // --- CONTINUED STATIC REGEX FILTERS (Only if both word lists and AI passed) ---
    // These are for specific non-toxic violations (ads, politics, etc.)
    
    // RULE: ANTI-HARASSMENT / ANTI-TROLLING (The explicit statements, which Static/AI might miss if they are too subtle)
    ...
    if (explicitTrollHarassRegex.test(lowerContent)) {
        ...
        if (member && member.manageable) {
            try {
                // Apply 60-minute timeout for explicit trolling/harassment
  ...
  // ==================== IMAGE ONLY CHANNEL LOGIC (TARGET_CHANNEL_ID) ====================
  if (message.channel.id === TARGET_CHANNEL_ID) {
    
    // Check 1: Attachments (images/videos uploaded)
    const hasAttachment = message.attachments.size > 0;

    // Check 2: Valid Media Links in the message content
    ...
    // If the message has NO attachments AND is NOT a media link, it must be deleted (Text Only).
    if (!hasAttachment && !isMediaLink) {
      await message.delete().catch(() => {});
      return;
    }
    // Message is kept if it has an image/media (with or without text).
